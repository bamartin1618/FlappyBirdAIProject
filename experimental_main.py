...
pygame.display.set_caption('Flappy Bird AI Experimental') 
...
if __name__ == '__main__':
    config_path = os.path.join(sys.path[0], 'experimental_config.txt')
    run(config_path)
    
    pickle.dump(best, open(os.path.join(sys.path[0], 'experimental_bird.pkl'), 'wb'))
    
    df = pandas.DataFrame(results)
    
    df.to_csv(os.path.join(sys.path[0], 'experimental_group_data.csv'))
