...
if __name__ == '__main__':
    config_path = os.path.join(os.path.join(sys.path[0], 'control_config.txt'))
    run(config_path)
    
    pickle.dump(best, open(os.path.join(sys.path[0], 'control_bird.pkl'), 'wb'))
    
    df = pd.DataFrame(results)
    
    df.to_csv(os.path.join(sys.path[0], 'control_group_data.csv'))
